...
class RealServerSentEvent implements ServerSentEvent {
    ...
    private void prepareCall(Request request) {
        ...
        Request.Builder requestBuilder = request.newBuilder().header("Accept-Encoding", "")
    ...
    private class Reader {
        ...
        private void processLine(String line) {
            ...
            } else if (colonIndex != -1) { // Collect the characters on the line after the first U+003A COLON character
                                           // (:), and let value be that string.
                String field = line.substring(0, colonIndex);
                String value = EMPTY_STRING;
                int valueIndex = colonIndex + 1;
                if (valueIndex < line.length()) {
                    if (line.charAt(valueIndex) == ' ') { // If value starts with a single U+0020 SPACE character,
