...
public interface ServerSentEvent {
    ...
    /**
     * Defines a timeout to close the connection when now events are received. This is useful to avoid hanging
     * connections.
     * Note if this is set a heartbeat mechanism should be implemented in server side to avoid closing connections when
     * no events.
     *
    ...
    /**
     * Force the Server Sent event channel to close. This will cancel any pending request or close the established
     * channel.
     */
    void close();
    ...
    interface Listener {
         ...
         * @return true if the connection should be retried after the defined retry time, false to avoid the retry, this
         *         will close the SSE.
         */
